...
def _visual_crop_cluster_near_label(
    ...
    try:
        ...
        # Column clamp: if label is clearly in one column, don't extend into the other.
...
def _crop_option_images(output: dict, extraction: dict, page_images: dict[int, str], option_dir: Path, exam_id: str, pdf_path: str | None):
    ...
    for q in output.get("questions", []):
        ...
        is_math_subject = "matem" in subject

        # Visual option crops are ONLY for questions whose statement explicitly asks
        # the student to choose a visual object. Do not crop short mathematical options
        # like "1", "2e", "e^2", "+inf".
        ...
        should_crop_options = (
            ...
            and (blank_or_label_only >= 3 or image_like_option_texts >= 3)
        )

        if is_math_subject and not visual_cue_in_statement:
            should_crop_options = False
